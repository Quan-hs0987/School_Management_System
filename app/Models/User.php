...
class User extends Authenticatable
{
    ...
    {
        $query = self::select('users.*')->where('user_type', '=', 1);

        // Kiểm tra xem trường is_delete có tồn tại trong bảng users
    ...
    static public function getParent(){
        ...
        if(!empty(Request::get('status'))){
            $status = (Request::get('status') == 100) ? 10 : 0;
    ...
    static public function getRecordFeesStudent()
    {
        ...
        $return = $return->orderBy('users.name', 'asc')->paginate(20);
        return $return;
    }
    ...
    static public function getTeacher(){
        ...
        if(!empty(Request::get('address'))){
            $return = $return->whereDate('users.address', 'like', '%'. Request::get('address').'%');
        }
        if(!empty(Request::get('admission_date'))){
            $return = $return->whereDate('users.admission_date', '=',  Request::get('admission_date'));
        }
    ...
    static public function getTeacherStudent($teacher_id)
    {
        ...
        $return = $return->orderBy('users.id', 'desc')->groupBy('users.id')->paginate(20);
        return $return;
    }
